...
int main(){
	...
	uint32_t pre_command = 0x00000000;
	uint8_t mode = 0x00;
	...
	while(1){
		...
		pre_command = AXI_PWM_mReadReg(COM_BASEADDR, COM_REG0);
		printf("Input: %02x %04lx  %04lx\n", mode, period, dead);
		if(mode== 0x0000){
			printf("reset: %08lx\n",pre_command&0xffffffff);
			AXI_PWM_mWriteReg(COM_BASEADDR, COM_REG0, 0x00000000);
		}else if(mode == 0x0001){
			AXI_PWM_mWriteReg(COM_BASEADDR, COM_REG0, ((pre_command&0xffff0000)|(command&0x0000ffff)));
			printf("Delay Only: %08lx\n",AXI_PWM_mReadReg(COM_BASEADDR, COM_REG0));
		}else if(mode==0x0010){
			AXI_PWM_mWriteReg(COM_BASEADDR, COM_REG0, ((pre_command&0x0000ffff)|(command&0xffff0000)));
